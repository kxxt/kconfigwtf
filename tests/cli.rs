...
#[test]
fn void_index_command_indexes_local_void_packages_checkout() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo_root = temp.path().join("void-packages");
    let package_dir = repo_root.join("srcpkgs/linux6.6");
    ...
    fs::write(
        files_dir.join("x86_64-dotconfig"),
        "# Linux/x86 6.6.141 Kernel Configuration\nCONFIG_VOID_TEST=y\n# CONFIG_UNUSED is not set\n",
    )
    .expect("write dotconfig");
    ...
    let config_path = data_dir.join("void/linux6.6/6.6.141_1/amd64/config");
    assert!(config_path.exists());
    assert!(
        fs::read_to_string(&config_path)
            .expect("read raw config")
            .contains("CONFIG_VOID_TEST=y")
    );

    let index_path = data_dir.join("void/linux6.6/index.json");
    let index: PackageIndex =
        serde_json::from_str(&fs::read_to_string(&index_path).expect("read output"))
            .expect("parse package index");
    assert_eq!(index.distribution, Distribution::Void);
    assert_eq!(index.package_name, "linux6.6");
    assert_eq!(
        index.kernels["6.6.141_1/amd64"].architecture,
        Architecture::Amd64
    );
    assert_eq!(
        index
            .entries
            .get("CONFIG_VOID_TEST")
            .expect("CONFIG_VOID_TEST")[0]
            .value,
        ConfigValue::BuiltIn
    );
    assert_eq!(
...
#[test]
fn openwrt_index_command_indexes_local_target_root() {
    let temp = tempfile::tempdir().expect("tempdir");
    let targets_root = temp.path().join("targets");
    let target_dir = targets_root.join("x86/64");
    let data_dir = temp.path().join("data");

    fs::create_dir_all(&target_dir).expect("create openwrt target");
    fs::write(
        target_dir.join("kernel.config"),
        "# Linux/x86 6.12.71 Kernel Configuration\nCONFIG_TEST=y\nCONFIG_FOO=y\n# CONFIG_UNUSED is not set\n",
    )
    .expect("write kernel config");
    fs::write(
        target_dir.join("profiles.json"),
        r#"{
  "arch_packages": "x86_64",
  "linux_kernel": { "version": "6.12.71" },
  "target": "x86/64",
  "version_number": "25.12.0",
  "version_code": "r32713-cbbcf1ec78"
}"#,
    )
    ...
    Command::cargo_bin("kconfigwtf")
        ...
        .args([
            ...
            "openwrt",
            "--targets-root",
            targets_root.to_str().expect("targets root"),
            "--target",
            "x86/64",
            "--data-dir",
            data_dir.to_str().expect("data dir"),
    ...
    let package_dir = data_dir.join("openwrt/x86-64");
    let config_path = package_dir.join("25.12.0-kernel-6.12.71/amd64/config");
    assert!(config_path.exists());
    assert!(
        fs::read_to_string(&config_path)
            .expect("read openwrt config")
            .contains("CONFIG_TEST=y")
    );

    let index: PackageIndex =
        serde_json::from_str(&fs::read_to_string(package_dir.join("index.json")).expect("index"))
            .expect("parse index");
    assert_eq!(index.distribution, Distribution::OpenWrt);
    assert_eq!(index.package_name, "x86-64");
    assert_eq!(
        index.kernels["25.12.0-kernel-6.12.71/amd64"].architecture,
        Architecture::Amd64
    );
    assert_eq!(
        index.entries.get("CONFIG_TEST").expect("CONFIG_TEST")[0].value,
        ConfigValue::BuiltIn
    );
}
...
#[test]
fn openkylin_index_command_indexes_local_packages_file() {
    apt_index_command_indexes_local_packages_file(AptCliCase {
        command: "openkylin",
        distribution: Distribution::OpenKylin,
        package_name: "linux-modules-6.6.0-15-generic",
        package_version: "6.6.0-15.0ok13",
        expected_config_path: "openkylin/linux-image-<VERSION>-generic/6.6.0-15.0ok13/amd64/config",
        expected_index_path: "openkylin/linux-image-<VERSION>-generic/index.json",
        expected_index_package_name: "linux-image-<VERSION>-generic",
        extra_packages: &[],
    });
}
